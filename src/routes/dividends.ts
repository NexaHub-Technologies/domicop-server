...
export const dividendRoutes = new Elysia({ prefix: '/dividends' })
  ...
  .get('/', async ({ query }) => {
    ...
    const { data, error } = await supabase.from('dividends')
    ...
    async ({ body, userId }) => {
      ...
      for (const dividend of body.dividends) {
        ...
        if (!member?.bank_account || !member?.bank_code) {
          results.push({ member_id: dividend.member_id, status: 'failed', error: 'Missing bank details' })
          continue
        }
        ...
        try {
          ...
          results.push({ member_id: dividend.member_id, status: 'processing', transfer_code: transfer.transfer_code })
        } catch (err) {
          results.push({ member_id: dividend.member_id, status: 'failed', error: (err as Error).message })
        }
      }
