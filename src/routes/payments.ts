...
export const paymentRoutes = new Elysia({ prefix: "/payments" })
  ...
  // Requires Paystack secret key authentication
  .use(paystackAuth)
  .post(
    "/verify",
    async ({ userId, body }) => {
  ...
  .post(
    ...
    async ({ userId, body }) => {
      ...
      try {
        ...
        if (verified.status === "success") {
          const updatedTransaction = await processSuccessfulPayment(
            reference,
            verified,
            userId,
          );
  ...
  // GET /payments/:ref/status - Check payment status (with optional force_refresh)
  // Requires Paystack secret key when force_refresh=true
  .use(paystackAuth)
  .get(
    "/:ref/status",
