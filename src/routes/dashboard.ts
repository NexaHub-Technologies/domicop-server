...
export const dashboardRoutes = new Elysia({ prefix: '/dashboard' })
  ...
  .get('/summary', async ({ userId }) => {
    ...
    const [profile, contributions, activeLoan, recentTxns, announcements] = await Promise.all([
      ...
      supabase.from('contributions')
        ...
        .eq('member_id', userId).eq('year', currentYear).eq('status', 'verified'),
      ...
      supabase.from('transactions')
        ...
        .eq('member_id', userId).eq('status', 'success')
    ...
    const totalSavings = contributions.data?.reduce((s, c) => s + Number(c.amount), 0) ?? 0
    const paidThisMonth = contributions.data?.some(c => c.month === currentMonth) ?? false
